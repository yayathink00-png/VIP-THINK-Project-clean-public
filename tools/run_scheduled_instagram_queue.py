...
def state_path_for(queue_path):
    ...
    return path.with_name(path.stem + ".instagram.state.json")
...
def load_cloudinary_fallbacks(queue_path):
    path = Path(queue_path).with_name(Path(queue_path).stem + ".batch-result.json")
    if not path.exists():
        return {}
    data = load_json(path)
    fallbacks = {}
    for item in data.get("facebook", []):
        key = f"{item.get('date')}#{item.get('sequence')}"
        image_url = item.get("image_url")
        if image_url:
            fallbacks[key] = image_url
    return fallbacks


def item_key(item):
    return f"instagram-global#{item['date']}#{item['sequence']}"


def caption_section(item, platform):
...
def main():
    parser = argparse.ArgumentParser(description="Run due VIP THINK Global Instagram scheduled posts.")
    ...
    image_fallbacks = load_cloudinary_fallbacks(queue_path)
    ...
    results = []
    for item in items:
        caption = extract_caption(preview_text, caption_section(item, "instagram"))
        image = item["image"]
        if not Path(image).is_file():
            image = image_fallbacks.get(f"{item['date']}#{item['sequence']}", image)
        result = publish_image_bundle(
            preset="global",
            image=image,
            global_caption=caption,
            publish_facebook_enabled=False,
